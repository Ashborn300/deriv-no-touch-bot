...
class NoTouchBot:
    ...
    async def health_check(self):
        try:
            ...
            logger.info(f"Connexion OK — {loginid} ({account_type})")
            logger.info(f"Solde disponible : {balance:.2f} {currency}")
            return True
    ...
    async def buy_notouch(self, sign):
        ...
        logger.info(f"🟢 Achat NOTOUCH {barrier} ({cid}) stake={STAKE}$ durée={DURATION_MINUTES}m")
        log_trade({
